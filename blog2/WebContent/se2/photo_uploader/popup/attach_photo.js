    ...
    function html5Upload() {	
    	...
    	sUploadURL= 'http://localhost:8080/malladmin/common/uploadSe2ImageHtml5'; 	//upload URL
 		...
 		try{
	    	...
	    	for (var i = 0; i < aTemp.length ; i++){
	    		if( !!aTemp[i] && aTemp[i] != "" && aTemp[i].indexOf("=") > 0){
	    			...
	    			htTemp[aSubTemp[0]] = aSubTemp[1].replace("[-]", "=");	// 2014.12.29 mykim : img src url에 사용하는 =이 splite("=") 때문에 비정상url 생성됨. [-]로 서버에서 받아온후 여기서 바꿔서 사용한다.
	    		}
	 		}
 		}catch(e){}
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		...
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  : 'http://localhost:8080/malladmin/common/uploadSe2Image',	//샘플 URL입니다.
